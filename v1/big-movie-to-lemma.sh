...
T=/tmp/movie.tmp
cat tv.cmd >> "$HOME/Videos/lemma/big.lemma"
go run pngs-to-coco-stream.go --header >> "$HOME/Videos/lemma/big.lemma"
go run pngs-to-coco-stream.go --header >> "$HOME/Videos/lemma/big.lemma"
#wc "$HOME/Videos/lemma/big.lemma"
for movie
do
	rm -rf $T
	mkdir -p $T
	...
	(
		go run pngs-to-coco-stream.go --concat --audio=$T/out.u8 $T/new.*.png
	) >> "$HOME/Videos/lemma/big.lemma"
done
